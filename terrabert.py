...
# 2. Создание токенизатора и обучение его на корпусе
def create_and_train_tokenizer(corpus_file, vocab_size=30000):
    """
    Создает и обучает токенизатор на корпусе текстов.
    """
    logging.info("Создание и обучение токенизатора...")
    # Загрузка предобученного токенизатора BERT (поддерживает кириллицу)
    tokenizer = BertTokenizer.from_pretrained("bert-base-multilingual-cased")

    # Сохранение токенизатора
    tokenizer.save_pretrained("my_bert_tokenizer")
    logging.info("Токенизатор сохранен в папку: my_bert_tokenizer")
...
if __name__ == "__main__":
    ...
    # 2. Создание и обучение токенизатора
